...
/**
 * Lane selector (mesh-map gap G4). Makes burn-governance actionable without a runtime:
 * a pure function that recommends which lane to use following the GPT-lane routing order
 *   eligibility -> privacy_fit -> task_fit -> cost_state -> recent_quality -> availability
 * with **task fit dominant and budget only a late gate** (budget decides between
 * near-equal fits, never overrides a clearly better one). It recommends; it does not run.
 */
...
export function selectLane(candidates: readonly LaneCandidate[]): LaneSelection {
  ...
  const ranked = acceptable
    ...
    .sort((left, right) => {
      // Among near-equal fits: healthier budget first, then fit, then recent quality.
