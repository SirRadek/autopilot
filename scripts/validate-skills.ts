...
export function detectAdapterDrift(core: SkillContract, manifest: AdapterManifest, markdown: string): string[] {
  const errors: string[] = []
...
if (invokedFile === fileURLToPath(import.meta.url)) {
  ...
  process.stdout.write(`${formatSkillsValidationReport(report)}\n`)
  if (!report.ok) {
    process.exit(1)
  }
}
