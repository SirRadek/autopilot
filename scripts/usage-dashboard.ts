...
import { buildUsageDashboard, type ProviderTarget, type UsageDashboard } from '@/lib/usage-dashboard'
import type { ProviderBudget, UsageLedgerEntry } from '@/lib/usage-ledger'
import { validateProviderBudget, validateUsageLedgerEntry } from '@/lib/usage-ledger'

// Starting-point target work-share (tune from the ledger over time).
...
export function renderUsageDashboardFromFiles(repoRoot: string, options: UsageDashboardCliOptions = {}): UsageDashboard {
  const ledgerPath =
    options.ledgerPath ??
    resolveFirst(repoRoot, ['.agent/usage/usage-ledger.jsonl', '.agent/usage/usage-ledger.example.jsonl'])
  const budgetPath =
    options.budgetPath ??
    resolveFirst(repoRoot, ['.agent/usage/provider-budget.json', '.agent/usage/provider-budget.example.json'])
...
function parseArgs(args: readonly string[]): UsageDashboardCliOptions & { format?: 'json' | 'markdown' } {
  const result: { project?: string; phase?: string; ledgerPath?: string; budgetPath?: string; format?: 'json' | 'markdown' } = {}
...
if (invokedFile === fileURLToPath(import.meta.url)) {
  ...
  if (args.format === 'json') {
    const json = { ...dashboard } as Record<string, unknown>
